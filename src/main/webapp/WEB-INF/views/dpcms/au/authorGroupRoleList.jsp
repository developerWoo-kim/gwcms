...
<script>
    $(document).ready(function() {
        ...
        $('.authorInsertPage').click(function(){
            $('#searchForm').attr('action','/admin/au/author-role/insertPage')
                .attr('method', 'POST').submit();
        });
    })

    /** 권한 삭제*/
    function authorGroupRoleDelete(authorGroupRoleId, authorGroupRoleName) {
        if (confirm(authorGroupRoleName + " 권한을 삭제하시겠습니까?")) {
            $.ajax({
                type : "DELETE",
                dataType : "json",
                url : "/admin/au/author-role/" + authorGroupRoleId,
    ...
    function goSearch() {
        $('#searchForm').attr('action','/admin/au/author-role/list')
            .attr('method', 'GET').submit();
    }
</script>
<!-- 권한 역할 관리 -->
<h4 class="tit1">권한 역할 목록</h4>
<div class="box_st1 mgb5">
    <p class="ac">권한 역할을 조회, 등록, 삭제할 수 있습니다.</p>
</div>

<div class="top_box">
    <form id="searchForm" name="searchForm" method="GET" >
        <input type="hidden" id="mi" name="mi" value="<c:out value='${condition.mi}'/>"/>
        <input type="hidden" id="authorGroupId" name="authorGroupId" value="<c:out value='${condition.authorGroupId}'/>"/>
        <div class="bbs_srch">
            ...
            <div class="srchBtn">
                <button type="button" title="권한등록" class="btnTy_blue01 authorInsertPage">역할등록</button>
            </div>
        </div>
    </form>
</div>
                ...
                <c:when test="${not empty pagination.content}">
                    <c:forEach var="list" items="${pagination.content}" varStatus="status">
                        <tr>
                            ...
                            <td class="ac"><c:out value="${list.authorGroupRoleName}"/></td>
                            <td class="ac">
<%--                                <a href="javascript:"  onClick="javascript:goViewUpdate('<c:out value="${list.authorGroupId}"/>');" class="btnTy_grn01">수정</a>--%>
                                <a href="javascript:"  onClick="javascript:authorGroupRoleDelete('<c:out value="${list.authorGroupRoleId}"/>', '<c:out value="${list.authorGroupRoleName}"/>');" class="btnTy_red">삭제</a>
                            </td>
                        </tr>
                    </c:forEach>
                </c:when>
