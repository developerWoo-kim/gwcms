...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(".menuInfo").click(function(){
			...
			$("#systemName").val($(this).attr("data-nm"));
	...
	function goSearch() {
		$('#searchForm').attr('action','/admin/mn/menu/sysList')
				.attr('method', 'GET').submit();
	}
</script>

<h4 class="tit1">메뉴 관리 - 홈페이지 목록
		...
		<tbody>
			<c:forEach items="${pagination.content}" var="sys" varStatus="status">
				<tr>
					...
					<td><button class="btnTy_blue01 menuInfo" data-id="<c:out value="${sys.systemId}"/>" data-nm="<c:out value="${sys.systemName}"/>" type="button">조회</button></td>
				</tr>
			</c:forEach>
		</tbody>
	</table>
