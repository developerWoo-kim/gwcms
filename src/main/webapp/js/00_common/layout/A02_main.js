$(function(){
  ...
  // popup
  $('#popupList').on('init', function (event, slick) {
    $('.A02_popup .nss_pg').prepend('<div class="slick-counter"><span class="current"></span> / <span class="total"></span></div>');
    $('.A02_popup .current').text(slick.currentSlide + 1);
    $('.A02_popup .total').text(slick.slideCount);
  });
  $("#popupList").slick({     
      infinite: true,
      autoplaySpeed:6000,
      ...
      $('.A02_popup .current').text(nextSlide + 1);
  });
    $('.A02_popup .control a').click(function(e){
      e.preventDefault();
      $this = $(this);
      slickControl($this, '#popupList','#popupStop','#popupPlay');
  });	
